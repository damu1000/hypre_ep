 ...
 *-------------------------------------------------------------------------*/
HYPRE_THREAD_LOCAL_EP bool        errFlag_dh = false; /* set to "true" by functions encountering errors */
HYPRE_THREAD_LOCAL_EP Parser_dh   parser_dh = NULL;   /* for setting/getting runtime options */
HYPRE_THREAD_LOCAL_EP TimeLog_dh  tlog_dh = NULL;     /* internal timing  functionality */
HYPRE_THREAD_LOCAL_EP Mem_dh      mem_dh = NULL;      /* memory management */
HYPRE_THREAD_LOCAL_EP FILE        *logFile = NULL;
#if defined(HYPRE_USING_RAJA) || defined(HYPRE_USING_KOKKOS) || defined(HYPRE_USING_CUDA)
#else
HYPRE_THREAD_LOCAL_EP char        msgBuf_dh[MSG_BUF_SIZE_DH]; /* for internal use */
#endif
HYPRE_THREAD_LOCAL_EP HYPRE_Int         np_dh = 1;     /* number of processors and subdomains */
HYPRE_THREAD_LOCAL_EP HYPRE_Int         myid_dh = 0;   /* rank of this processor (and subdomain) */
HYPRE_THREAD_LOCAL_EP MPI_Comm    comm_dh = 0;
...
HYPRE_THREAD_LOCAL_EP bool logInfoToStderr  = false;
HYPRE_THREAD_LOCAL_EP bool logInfoToFile    = true;
HYPRE_THREAD_LOCAL_EP bool logFuncsToStderr = false;
HYPRE_THREAD_LOCAL_EP bool logFuncsToFile   = false;

HYPRE_THREAD_LOCAL_EP bool ignoreMe = true;
HYPRE_THREAD_LOCAL_EP HYPRE_Int  ref_counter = 0;
...
static HYPRE_THREAD_LOCAL_EP  char errMsg_private[MAX_STACK_SIZE][MAX_MSG_SIZE];
static HYPRE_THREAD_LOCAL_EP  HYPRE_Int errCount_private = 0;

static HYPRE_THREAD_LOCAL_EP  char calling_stack[MAX_STACK_SIZE][MAX_MSG_SIZE];
/* static HYPRE_THREAD_LOCAL_EP  HYPRE_Int  priority_private[MAX_STACK_SIZE]; */
static HYPRE_THREAD_LOCAL_EP  HYPRE_Int calling_stack_count = 0;

/* static HYPRE_THREAD_LOCAL_EP  char errMsg[MAX_MSG_SIZE];    */
...
static HYPRE_THREAD_LOCAL_EP char spaces[MAX_ERROR_SPACES];
static HYPRE_THREAD_LOCAL_EP HYPRE_Int nesting = 0;
static HYPRE_THREAD_LOCAL_EP bool initSpaces = true;
#define INDENT_DH 3
...
static HYPRE_THREAD_LOCAL_EP bool EuclidIsActive = false;
