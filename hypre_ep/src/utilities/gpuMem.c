...
HYPRE_THREAD_LOCAL_EP HYPRE_Int hypre_exec_policy = HYPRE_MEMORY_DEVICE;
#endif
...
HYPRE_THREAD_LOCAL_EP struct hypre__global_struct hypre__global_handle = {0, 0, 1, 0};
...
cublasHandle_t getCublasHandle(){
  cublasStatus_t stat;
  static HYPRE_THREAD_LOCAL_EP cublasHandle_t handle;
  static HYPRE_THREAD_LOCAL_EP hypre_int firstcall=1;
  if (firstcall){
    firstcall=0;
...
cusparseHandle_t getCusparseHandle(){
  ...
  static HYPRE_THREAD_LOCAL_EP cusparseHandle_t handle;
  static HYPRE_THREAD_LOCAL_EP hypre_int firstcall=1;
  if (firstcall){
    firstcall=0;
...
cudaStream_t getstreamOlde(hypre_int i){
  ...
  static HYPRE_THREAD_LOCAL_EP cudaStream_t s[MAXSTREAMS];
  if (firstcall){
    hypre_int jj;
...
nvtxDomainHandle_t getdomain(hypre_int i){
    ...
    static HYPRE_THREAD_LOCAL_EP nvtxDomainHandle_t h[MAXDOMAINS];
    if (firstcall){
...
cudaEvent_t getevent(hypre_int i){
  ...
  static HYPRE_THREAD_LOCAL_EP cudaEvent_t s[MAXEVENTS];
  if (firstcall){
    hypre_int jj;
...
hypre_int getsetasyncmode(hypre_int mode, hypre_int action){
  static HYPRE_THREAD_LOCAL_EP hypre_int async_mode=0;
...
size_t mempush(const void *ptr, size_t size, hypre_int action){
  static HYPRE_THREAD_LOCAL_EP node* head=NULL;
  static HYPRE_THREAD_LOCAL_EP hypre_int nc=0;
  node *found=NULL;
  if (!head){
    if ((size<=0)||(action==1)) {
