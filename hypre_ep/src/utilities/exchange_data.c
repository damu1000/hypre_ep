                                 ...
                                 HYPRE_Int **p_response_recv_buf_starts)
{
   ...
   while (!terminate)
   {
      ...
      //hypre_MPI_Iprobe(hypre_MPI_ANY_SOURCE, contact_tag, comm, &contact_flag, &status);
	  MPI_Message 	message;	//damodar
	  hypre_MPI_Improbe(hypre_MPI_ANY_SOURCE, contact_tag, comm, &contact_flag, &message, &status);
      ...
      while (contact_flag)
      {
         ...
         //hypre_MPI_Recv(recv_contact_buf, contact_size*contact_obj_size, hypre_MPI_BYTE, proc, contact_tag, comm, &fill_status);
         hypre_MPI_Mrecv(recv_contact_buf, contact_size*contact_obj_size, hypre_MPI_BYTE, &message, &fill_status);		//damodar
